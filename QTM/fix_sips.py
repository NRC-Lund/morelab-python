...
from helpers.traj import get_default_markerset_marker, get_selected_markerset_marker, get_labeled_marker_ids, get_unlabeled_marker_ids, get_marker_positions
...
def check_trajectory_continuity(id):
    ...
    if traj_length['start'] != 0 or frames['end'] != traj_length['end'] or gap_ranges:
        print(f"The {qtm.data.object.trajectory.get_label(id)} trajectory is not continuous. Please check the measurement.")
        return False
    else:
        return True
...
if __name__ == '__main__':
    add_command("fix_sips", fix_sips)
    menu_id = qtm.gui.insert_menu_submenu(None,"MoreLab")
    add_menu_item(menu_id, "Fix SIPS labels", "fix_sips")